...
# Last Modified:  Tuesday, 18th February 2020 2:25:36 pm
...
from training.learn_ours import learnD_Realness,learnG_Realness
...
_,runingLabel2 = Sampler.prepare_z_c(param.batch_size, param.z_size, param.n_class)
...
total_points = param.n_class*sampleBatch
...
for i in range(iter_offset, param.total_iters):
    ...
    lossD = learnD_Realness(param, D, G, optimizerD, wocao, runingZ, runingLabel,runingLabel2)
    lossG = learnG_Realness(param, D, G, optimizerG, wocao, runingZ, runingLabel,runingLabel2)
    ...
    if (i+1) % param.gen_every == 0:
        ...
        with torch.no_grad():
            ...
            # plt.xlabel('X')
            # plt.ylabel('Y')
            for class_i in range(param.n_class):
                x=[]
                y=[]
                for dot in range(sampleBatch):
                    x.append(fake_test_list[class_i*sampleBatch+dot][0])
                    y.append(fake_test_list[class_i*sampleBatch+dot][1])
                fig = plt.figure(figsize=(10,10))
                ax1 = fig.add_subplot(111)
                ax1.set_title('Points distribution')
                plt.xlabel('X')
                plt.ylabel('Y')
                ax1.scatter(x,y,c = color_table[class_i],marker = '.',alpha=0.5,edgecolors='k')
                plt.savefig(sample_path +'/test%d-class_%d.png'%((i+1),class_i),bbox_inches = 'tight')